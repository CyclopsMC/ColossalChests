...
public class ItemUpgradeTool extends ConfigurableItem {
    ...
    @Override
    public EnumActionResult onItemUseFirst(ItemStack itemStack, EntityPlayer player, final World world, BlockPos pos,
        ...
        while (it.hasNext()) {
            ...
            if (stack != null) {
                if (ItemStack.areItemsEqual(stack, consumeStack)) {
                    ...
                    validItems += stack.stackSize;
                    validItems = Math.min(consumeStack.stackSize, validItems);
                    if (!simulate) {
                        stack.stackSize -= validItems - previousValidItems;
                        it.replace(stack.stackSize == 0 ? null : stack);
                    }
                }
            }
        }
        return validItems == consumeStack.stackSize;
    }
